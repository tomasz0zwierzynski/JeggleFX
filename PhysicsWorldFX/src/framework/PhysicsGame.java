...
public class PhysicsGame {
    ...
    private final float fps = 80f;
    private float timeStep = 1000f / fps;

    private final float sps = 100f;
    private final float spsStep = 1000f / sps;
    private final int physicsStepMs = 10;

    private int currentStep = 0;
    private boolean drawDebug;
    private boolean isUpdating;

    private Map<Node, Body> nodeBodyMap = new ConcurrentHashMap<>();
	...
	public void load(Region gameContainer){
		...
		if(this.gameWorld != null){
            ...
            physicTimer = new Timer();
            physicTimer.schedule(new TimerTask() {
                @Override
                public void run() {
                    ...
                    world.step(spsStep / 1000f, 8, 3);

                    EventHandler<? super PhysicsEvent> handler = gameWorld.getOnPhysicsStep();
                    if (handler != null){
                        currentStep = currentStep == sps ? 1 : ++currentStep;
                        handler.handle(new PhysicsEvent(PhysicsEvent.PHYSICS_STEP, currentStep, sps));
                    }

                    // System.out.println("physic: " + ( (System.nanoTime() - TIME_START) / 1000) + " us");
                }
            }, 0, physicsStepMs);
			...
			worldTimer = animationTimerFactory.CreateTimer(timeStep,t -> {
                ...
                // System.out.println("animationTimer: " + ( (System.nanoTime() - TIME_START) / 1000) + " us");
			});
